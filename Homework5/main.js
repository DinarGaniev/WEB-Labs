...
function actionTaskBtnHandler(event) {
    ...
    if (action == 'create') {
        ...
        postTask(form)
            ...
            .then(array => showAlert(alertMsg = `Задача ${array.name} была успешно создана!`, 'success'));
...
function prepareModalContent(event) {
    ...
    if (action == 'edit' || action == 'show') {
        viewTask(event.relatedTarget.closest('.task').id)
            .then(jsonData => setFormValues(form, jsonData));
...
} // смена статуса при нажатии на стрелочку перемещения 
